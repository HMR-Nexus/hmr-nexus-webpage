...
export function Navbar({ activePage, onNavigate }: NavbarProps) {
	...
	const navigate = useCallback(
		(page: PageId, projectType?: string) => {
			...
			>
				<div className="max-w-[1440px] mx-auto px-6 md:px-7 py-3.5 flex items-center justify-between">
					...
					<div className="flex items-center gap-4">
						{/* Locale switcher */}
						<div
							className="hidden md:flex items-center gap-1.5 mono-tag text-paper"
							role="group"
			...
			<AnimatePresence>
				{isMobileMenuOpen && (
					<motion.div
						...
						className="fixed inset-0 z-40 bg-ink pt-20 pb-8 overflow-y-auto lg:hidden"
						role="dialog"
						aria-modal="true"
					...
					>
						<div className="px-6 py-8 flex flex-col gap-6">
							...
							<div
								className="rule-top pt-6 mt-2 flex items-center gap-3 mono-tag text-paper"
								role="group"
								aria-label="Select language"
							>
								{languages.map((lng, i) => {
